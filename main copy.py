...
async def capture_screenshot(request: VideoRequest):
    ...
    while current_try < max_retries:
        try:
            current_try += 1
            print(f"\nScreenshot attempt {current_try} of {max_retries}")
            ...
            async with async_playwright() as p:
                ...
                print(f"Navigating to video {request.video_id} at {request.timestamp}...")
                
                # Use embed URL with autoplay and start time
                url = f"https://www.youtube.com/embed/{request.video_id}?start={int(request.timestamp)}&autoplay=1"
                await page.goto(url, wait_until='networkidle')
                
                # Wait for video element with a basic selector
                await page.wait_for_selector('video', state='attached')
                
                # Force video element to be visible and wait for it to load
                await page.evaluate("""
                    ...
                    const overlay = document.querySelector('.ytp-chrome-top');
                ...
                # Set exact time and ensure video is ready
                ...
                # Remove all YouTube overlays
                await page.add_style_tag(content="""
                    .ytp-chrome-bottom, .ytp-chrome-top, .ytp-gradient-bottom,
                    .ytp-gradient-top, .ytp-pause-overlay, .ytp-spinner,
                    iframe[id^='player_uid_'] { opacity: 0 !important; }
                    .html5-video-container { opacity: 1 !important; }
                    video { 
                        opacity: 1 !important;
            ...
            print(f"Screenshot attempt {current_try} failed:")
